...
- Исследование товара через OpenAI Responses передаёт провайдеру реальные детерминированные ограничения: не более 8 встроенных Web Search-вызовов (max_tool_calls) и не более 16 000 выходных токенов (max_output_tokens) на ответ. Это оставляет агенту достаточно попыток для 3–6 точных источников, но не позволяет одному поиску разрастись до десятков вызовов, исчерпать TPM организации и затем целиком повториться после rate limit.
...
- Обязательная последовательность: Web Search выбирает один логический товарный источник → обычный HTML-сборщик дешёво собирает прямые URL и признаки галереи как входные данные → обязательный AI-предфильтр оценивает страницу → при наличии реальной галереи Playwright по умолчанию обучает или исполняет лучший доступный рецепт → техническая проверка и Vision подтверждают результат. Статичные URL являются seed/fallback и не должны безусловно отключать Playwright своим количеством.
- Playwright является приоритетным способом получения полной галереи почти для каждого доступного источника с признаками галереи. Он не запускается только когда предфильтр обоснованно вернул `no_gallery`/`blocked`, источник недоступен, исчерпан бюджет либо оператор явно отключил Playwright-first в Filament.
...
- Настройка Filament «Обучать Playwright даже когда статики "достаточно"» по умолчанию включена. Поэтому вердикт `static_sufficient` при найденной настоящей галерее (`gallery_likely`) не считается окончательным: рецепт всё равно обучается, реально исполняется и проверяется Vision. Отключение настройки является явным дешёвым режимом, а не стандартным поведением.
...
- Активировать рецепт можно после фактической проверки галереи и получения не менее настраиваемого в Filament минимума проверенных изображений (по умолчанию 3, максимум 10) — не обязательно всей галереи целиком; это порог успеха, а не квота.
...
- Как только Vision подтвердил хотя бы одно фото из Playwright-подтверждённой галереи/слайдера конкретной страницы, остальные фото из этой же галереи принимаются без отдельного запроса к Vision (сайты не смешивают в одном слайдере разные товары). Доверие ограничено одной галереей одного источника в рамках одного поиска и не переносится на другой источник или следующий цикл. Оператор всё равно проверяет черновик перед добавлением в каталог и может отклонить фото вручную.
...
- Основная защита от дубля «то же фото, другой размер» — нормализация URL до скачивания (по известным CDN-паттернам: Adobe Scene7 `wid/hei`/именованный пресет, Amazon `._SXNN_`, ASUS CDN, Shopify `width`/`height` query и Shopify filename-суффикс `_180x`/`_grande`/`_1920x`/`@2x`). Perceptual hash после скачивания — не основная защита, а подстраховка: на реальном кейсе (vishalperipherals.com, Shopify `&width=1445`) расстояние хэша между оригиналом и ресайзом составило 7 при пороге 6 — то есть мимо на 1 бит, и дедуп по хэшу сравнивает только внутри одного прохода/батча, а не по всему черновику целиком (статичный проход и принудительный интерактивный ретрай — разные вызовы). Каждый новый сайт может использовать свою ещё не описанную схему ресайза URL — это открытый список, а не разовая починка одного сайта. Правила URL-ключа продублированы в двух местах и обязаны совпадать: `scripts/product-gallery-utils.mjs` (`imageAssetKey`/`urlQualityScore`/`withUpscaledSizeParam`, JS-раннер Playwright) и `App\Services\Products\ProductImageStorage::normalizeCandidateUrl()` (PHP, общий resolve-пайплайн).
...
- Загрузка фото всегда завершается явной итоговой строкой («📦 Итог: N фото из статичной HTML-галереи / через AI-переобучение Playwright-рецепта / через резервный поиск», с пометкой частичного результата) — промежуточные шаги (в т.ч. неудачные, вроде отказа Playwright) не должны оставлять немую яму между последней ошибкой и финальным «черновик готов».
